...
class AlnajahScanClient {
  ...
  Future<ScanUser> login({required String username, required String password}) async {
    ...
    final data = r.data ?? {};
    if (data['ok'] != true) {
      throw AlnajahScanException(data['error']?.toString() ?? 'Login failed', r.statusCode);
    }
    final u = data['user'] as Map<String, dynamic>?;
    if (u == null) {
      throw AlnajahScanException('Invalid login response', r.statusCode);
    }
  ...
  Future<MeResponse> me() async {
    ...
    final data = r.data ?? {};
    if (data['ok'] != true) {
      throw AlnajahScanException(data['error']?.toString() ?? 'Not authenticated', r.statusCode);
    }
  ...
  Future<List<StockTakeSessionSummary>> listStockTakeSessions() async {
    ...
    final data = r.data ?? {};
    if (data['ok'] != true) {
      throw AlnajahScanException(data['error']?.toString() ?? 'Request failed', r.statusCode);
    }
...
class AlnajahScanException implements Exception {
  AlnajahScanException(this.message, this.statusCode);
  final String message;
  final int? statusCode;

  @override
  String toString() => 'AlnajahScanException($statusCode): $message';
}
